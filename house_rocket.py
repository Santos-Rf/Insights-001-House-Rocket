...
def portfolio_density(data, geofile):
    ...
    c1.header('Densidade do Portfolio')
...
def hipoteses (df):
  ...
  h4 = df[['price', 'renovacao', 'sqft_lot']].groupby('renovacao').mean().reset_index()
  fig4 = px.bar(h4, x='renovacao', y = 'price', color = 'renovacao', labels = {'renovacao':'Renovação',
  ...
  h51 = h5[['price', 'view', 'sqft_lot']].groupby('view').sum().reset_index()

  fig5 = px.bar(h51, x= 'view', y = 'price', color = 'view', labels = {'price':'Preço','view': 'Qualidade da Vista'},
  template = 'simple_white')
  ...
  'atual')
  h6 = df[df['renovacao'] == 1]
  h6 = df[['contrucao', 'price', 'sqft_lot']].groupby('contrucao').sum().reset_index()
  fig6 = px.bar(h6, x ='contrucao', y = 'price', color = 'contrucao', labels = {'price':'Preço','contrucao': 'Tempo de Imóveis não renovados'} ,
  template= 'simple_white')
  ...
  h7 = df[['banheiro', 'price', 'sqft_lot']].groupby('banheiro').mean().reset_index()
  fig7 = px.bar(h7, x = 'banheiro', y = 'price', color = 'banheiro', labels = {'price':'Preço','banheiro':
  ...
  h8 = df[['contrucao', 'price', 'sqft_lot']].groupby('contrucao').mean().reset_index()

  fig8 = px.bar(h8, x = 'contrucao', y = 'price', color = 'contrucao', labels = {'price':'Preço', 'contrucao': 'Tempo de renovação'},
  template = 'simple_white')
  ...
  h9 = year_df[['mes', 'price', 'sqft_lot']].groupby('mes').sum().reset_index()
  fig9 = px.line(h9, x='mes', y = 'price', color_discrete_sequence= ['teal'], template = 'simple_white',
  ...
  h10 = df[(df['bathrooms'] == 3)]

  h10 = h10[['mes', 'price', 'sqft_lot']].groupby('mes').sum().reset_index()


  fig10 = px.line(h10, x = 'mes', y = 'price', color_discrete_sequence= ['teal'], template = 'simple_white',
  labels= {'mes':'Mês', 'price': 'Preço'})

  st.plotly_chart(fig10, x='mes', y='price', use_container_width= True)
...
def questoes_negocio (df):
  ...
  for i, row in df5.iterrows():
    if (row['price_medi_season'] > row['price']):
      df5.loc[i, 'sale'] = row['price'] * 0.1 + row['price']
    else:
      df5.loc[i, 'sale'] = row['price'] * 0.3 + row['price']
  ...
  st.markdown("<h1 style='text-align: center; color: black;'> Lucro Gerado.</h1>", unsafe_allow_html=True)

  lucro = df5.assign(lucro = df5['sale'] - df5['price'])
  lucro = lucro['lucro'].sum()
  
  st.text("O lucro gerado neste projeto é de R$ {}".format(lucro))
